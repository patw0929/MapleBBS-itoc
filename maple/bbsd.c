...
tn_login()
{
  ...
  if (cuser.userlevel)		/* not guest */
  {
   ...
   move(b_lines - 2, 0);
   prints("★ 我記得那天是 \033[1;33m%s\033[m",
          Btime(&cuser.lastlogin));
...
tn_main()
{
  ...
  prints("%s ⊙ " SCHOOLNAME " ⊙ " MYIPADDR "\n"
    ...
    "系統負載 %.2f %.2f %.2f [%s]",
    str_host, str_site, ushm->count, load[0], load[1], load[2],
