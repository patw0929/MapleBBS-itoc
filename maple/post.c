                                   ...
                                   /* 置底文不顯示分數 */
  {
    num = hdr->score;
     if (num > 98)
       ...
       prints("\033[1;33m%2d\033[m ", num);
     else if (num > 0)
       prints("\033[1;31m%2d\033[m ", num);
     else if (num < -98)
       prints("\033[1;30mXX\033[m ");
     else if (num < -10)
       prints("\033[1;30mX%d\033[m ", abs(num)/10);
     else
       prints("\033[0m  \033[m ");
   }
   else
   ...
   hdr_outs(hdr, d_cols + 44);  /* 少一格來放分數 */
 #else
   prints("%6d%c%c ", (hdr->xmode & POST_BOTTOM) ? -1 : num, tag_char(hdr->chro, post_attr(hdr));
  ...
  XO *xo;
{
  ...
  xbattr2 = (bshm->bcache + xbno)->readlevel;
  if (!HAS_PERM(PERM_ALLADMIN))
  {
    time_t now;
    struct tm *ptime;
    ...
    if((xbattr2==PERM_SYSOP) || (xbattr2==PERM_BOARD))
  ...
  XO *xo;
{
 if (bbstate & STAT_BOARD) /* bm only */
 {
    ...
    switch (vans("◎評分設定 1)自訂 2)清除 3)爆 4)XX [Q] "))
    /* 補強精華區function :) */
    {

    case '2':
      hdr->xmode &= ~POST_SCORE; /* 簡化: 歸零則去除POST_SCORE */
      hdr->score = 0;
      currchrono = hdr->chrono;
      break;
    ...
    case '1':
      ...
      if (pm != '1' && pm != '2')
        return XO_FOOT;
      if (!vget(b_lines, 0, "請輸入數字：", score, 3, DOECHO))
        return XO_FOOT;
      hdr->xmode |= POST_SCORE; /* 原文可能無評分 */
      hdr->score = pm == '1' ? atoi(score) : -atoi(score); /* 正負值判斷 */
      currchrono = hdr->chrono;
      break;
    ...
    rec_put(xo->dir, hdr, sizeof(HDR), xo->key == XZ_XPOST ? hdr->xid
    : pos, cmpchrono);
    move(3 + cur, 7);
    outc(post_attr(hdr));
   }
  ...
  XO *xo;
{
  ...
  char *dir, *userid, *verb, fpath[64], reason[80], verb_buf[3];
  FILE *fp;
#ifdef HAVE_ANONYMOUS
  char uid[IDLEN + 1];
#endif
...
{
 next = 1;
   switch (ans = vans("\033[1;37;40m您覺得這篇文章\033[m \033[1;33m1)絕對要推\033[m \033[1;31m2)噓給他死\033[m \033[1;37;40m3)只加→註解\033[m [1]? "))
   {
   case '1':
     verb = "\033[1;37m推";
     vtlen = 2;
...
else
{
 ...
 vtlen = 2;
  ...
  if (currbattr & BRD_ANONYMOUS)
    ...
    strcpy(&verb_buf[0], (verb+7));
    sprintf(fpath, "%s %s：", verb_buf, cuser.userid);

if (next == 1)
{
  if (vget(b_lines, 0, fpath, reason, maxlen, DOECHO))
  {
  if (vans("確定？ [y/N] ") != 'y')
    return XO_FOOT;
  }
...
else
{
 ...
 else
 {
  ...
  if (fp = fopen(fpath, "a"))
  {
    ...
    fprintf(fp, "%s\033[m \033[36m%s\033[m：%-*s%02d/%02d %02d:%02d\n",
...
static void
send_him_mail(userid, gold, money) /* itoc.011115: 寄檔案給 userid */
  char *userid;        /* 收件人 */
  int gold, money;
{
...
KeyFunc post_cb[] =
{
  ...
  /* 'X', post_forward, */      /* 取消 X 轉寄功能，X 改為推文，% 改為轉寄給使用者 */
  '%', post_forward,
  't', post_tag,
  'E', post_edit,
  'T', post_title,
