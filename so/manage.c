...
XO *xo;
{
   int i;
   FILE *fp_prefix;
   char fpath_prefix[64],
        fpath_template[64],
        buf[20], prefix[NUM_PREFIX][20], *menu[NUM_PREFIX + 3];
   char ftemplate[11];
   char *prefix_def[NUM_PREFIX] =   /* 預設的類別 */
   {
      "公告", "測試", "閒聊", "灌水", "無聊", "打混"
   ...
   brd_fpath(fpath_prefix, currboard, "prefix");

   i = 0;

   if (fp_prefix = fopen(fpath_prefix, "r"))
   {
      for (; i < NUM_PREFIX; i++)
      {
         if (fscanf(fp_prefix, "%10s", buf) != 1)
            break;
         sprintf(prefix[i], "%d.%s", i + 1, buf);
      }
      fclose(fp_prefix);
   ...
   menu[NUM_PREFIX + 2] = NULL;
    ...
    /* 在 popupmenu 裡面按 左鍵 離開 */
    i = pans(3, 20, "文章範本", menu) - '0';

    if (i >= 1 && i <= NUM_PREFIX)
    {
        sprintf(ftemplate, "%s_%d", FN_POSTTEMPLATE, i);
        brd_fpath(fpath_template, currboard, ftemplate);

        if (vedit(fpath_template, 0))        /* Thor.981020: 注意被talk的問題
 */
            vmsg(msg_cancel);
        return XO_HEAD;
    }
